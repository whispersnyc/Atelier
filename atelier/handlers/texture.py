...
from atelier.config import ASSETS, ASSETS_MODS, PAKS, USMAP, _WORK, check_prereqs
from atelier.tools import uat, uat_json
from atelier.paths import char_id, game_rel_for_skin, skin_entries, filter_subpath, skin_rel
...
def cmd_import(arg):
    ...
    cid       = char_id(skin_id)
    dest_root = os.path.abspath(os.path.join(ASSETS, "Marvel", "Content", "Marvel",
                                              "Characters", cid, skin_id))
    ...
    for arg in args:
        ...
        if re.match(r"^\d{7}(/|$)", noext):
            ...
            if "*" in tex_part:
                ...
                cid      = char_id(skin_id)
                skin_dir = os.path.join(ASSETS, "Marvel", "Content", "Marvel",
                                        "Characters", cid, skin_id)
